...
def serialize_post_optimized(post):
    return {
        ...
        'published_at': post.published_at,
        'slug': post.slug,
        'tags': [serialize_tag_optimized(tag) for tag in post.tags_prefetched],
...
def index(request):
    prefetch = Prefetch('tags', queryset=Tag.objects.popular(), to_attr='tags_prefetched')
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.prefetch_related(prefetch).fetch_with_comments_count()[:20]
    context = {
        'tag': tag.title,
        'popular_tags': [serialize_tag_optimized(tag) for tag in most_popular_tags],
